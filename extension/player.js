import {normalizeSettings, outputMix, TRANSLATED_CAPTION_DELAY_SECONDS, updateOutputMix} from './core.mjs';
...
function preciseWarningKey(code){if(code==='groq_auth_failed')return'groqAuthFailed';if(code==='groq_access_forbidden')return'groqAccessFailed';if(code==='groq_connection_failed')return'groqConnectionFailed';if(code==='groq_quota_exceeded')return'groqQuotaFailed';if(code==='gemini_voice_auth_failed')return'voiceAuthFailed';if(code==='gemini_voice_quota_exceeded')return'voiceQuotaFailed';return'warning';}
...
function updateCaptions(){
  ...
  const translatedNow=Math.max(0,translatedClock-TRANSLATED_CAPTION_DELAY_SECONDS);
...
channel.onmessage=({data})=>{
  ...
  if(data?.type==='warning'){$('#warningText').textContent=t(preciseWarningKey(data.error));$('#playerWarning').hidden=false;}
};
...
video.addEventListener('timeupdate',()=>{if(Number.isFinite(lastStallPosition)&&Math.abs(video.currentTime-lastStallPosition)>.1){lastStallPosition=video.currentTime;repeatedStalls=0;}captionsDirty=true;updateProgress();updateCaptions();scheduleWindow();if(dubbedTrack?.src&&!dubbedTrack.paused&&Math.abs(dubbedTrack.currentTime-video.currentTime)>.12)dubbedTrack.currentTime=video.currentTime;});for(const event of ['waiting','stalled'])video.addEventListener(event,()=>runAction(enterRebuffer));video.addEventListener('playing',()=>{clearStageError();$('#liveBadge span').textContent=t('playing');$('#rebufferNotice').hidden=true;scheduleWindow();});
