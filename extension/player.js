...
function preciseWarningKey(code){if(code==='groq_auth_failed')return'groqAuthFailed';if(code==='groq_quota_exceeded')return'groqQuotaFailed';if(code==='gemini_voice_auth_failed')return'voiceAuthFailed';if(code==='gemini_voice_quota_exceeded')return'voiceQuotaFailed';return'warning';}
...
function updateCaptions(){
  ...
  const now=video.currentTime;const finalDubTime=Number(dubbedTrack?.currentTime);const translatedNow=finalizedPlayback&&dubbedTrack?.src&&mix.dubAudioEnabled
    ? (Number.isFinite(finalDubTime)?finalDubTime:now)
    : Math.max(0,now-Math.max(.04,.025+(Number(audioContext?.outputLatency)||Number(audioContext?.baseLatency)||0)));
