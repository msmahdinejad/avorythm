...
function initialAudioReady(){
  const originalAudible=Boolean(settings.originalAudioEnabled&&Number(settings.originalVolume)>0);
  const dubAudible=Boolean(settings.dubAudioEnabled&&Number(settings.dubVolume)>0);
  if(originalAudible||!dubAudible)return true;
  const end=playableEnd();
  return Boolean(dubbedTrack?.src||dubTimeline.some((chunk)=>chunk.start+chunk.duration>video.currentTime+.01&&chunk.start<=end+.08));
}
...
function updateProgress(){
  ...
  const videoReady=ahead>=bufferTarget-.35||streamEnded&&ahead>.1;const ready=videoReady&&initialAudioReady();$('#activateButton').disabled=!ready;$('#activateButton').classList.toggle('ready',ready);void maybeResume();
}
...
channel.onmessage=({data})=>{
  ...
  if(data?.type==='dub-chunk'){const id=data.id||`dub-${++dubSequence}`;if(dubTimeline.some((chunk)=>chunk.id===id))return;const chunk={...data,id,duration:Number(data.duration)||new Int16Array(data.data).length/24000};dubTimeline.push(chunk);updateProgress();scheduleDub(chunk);return;}
