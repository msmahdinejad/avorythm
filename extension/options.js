...
const copy = {
  fa: {
    ...
    syncPlayer:'ضبط و پلیر هماهنگ',syncHelp:'ضبط تب جلوتر از پلیر ادامه پیدا می‌کند تا پخش، Seek و دوبله پایدار بمانند.',buffer:'فاصلهٔ ضبط تا پخش',bufferHelp:'ضبط جلوتر از پلیر ادامه پیدا می‌کند؛ ۲۰ ثانیه برای پایداری پیشنهاد می‌شود.',faster:'شروع سریع‌تر',steadier:'هماهنگی پایدارتر',timingEngine:'زمان‌بندی زیرنویس',geminiTiming:'Gemini Live · سریع و بدون کلید اضافه',whisperTiming:'Groq Whisper · زمان‌بندی دقیق‌تر',timingEngineHelp:'در حالت Whisper، دوبله همچنان با Gemini 3.5 Live ساخته می‌شود.',clearGroqKey:'پاک‌کردن کلید Groq این نشست',groqKeyReady:'کلید Groq برای این نشست آماده است.',groqKeyMissing:'برای حالت Whisper یک کلید Groq وارد کن.',
  ...
  en: {
    ...
    syncPlayer:'Synchronized recorder & player',syncHelp:'The tab keeps recording ahead of playback so seeking and dubbing remain stable.',buffer:'Recording lead',bufferHelp:'Capture stays ahead of playback; 20 seconds is recommended for stability.',faster:'Faster start',steadier:'Steadier sync',timingEngine:'Caption timing',geminiTiming:'Gemini Live · fast, no extra key',whisperTiming:'Groq Whisper · more precise timing',timingEngineHelp:'Whisper only supplies timestamps; Gemini 3.5 Live still creates the dub.',clearGroqKey:'Clear Groq session key',groqKeyReady:'The Groq key is ready for this session.',groqKeyMissing:'Add a Groq key to use Whisper timing.',
...
function render(){
  ...
  $('#groqConnection').classList.toggle('required',settings.syncCaptionEngine==='whisper'&&!groqKeySet);
...
$('#saveGroqKeyButton').addEventListener('click',async()=>{
  ...
  if(!granted){notice(t('groqPermissionDenied'));return;}
...
$('#clearGroqKeyButton').addEventListener('click',async()=>{await chrome.runtime.sendMessage({type:'clear-groq-key'});await chrome.permissions.remove({origins:['https://api.groq.com/*']});groqKeySet=false;renderKey();notice(t('keyCleared'),true);});
