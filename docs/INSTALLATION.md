...
1. Download `Dubira-Setup-x64.exe` from Releases.
...
4. Launch Dubira and open **Advanced settings**.
...
Restart Dubira after installing FFmpeg.
...
1. Download `Dubira-Extension.zip` and extract it into a permanent folder. Do not select the ZIP itself.
2. Open `chrome://extensions` or `edge://extensions`.
3. Enable **Developer mode**.
4. Click **Load unpacked** and select the folder that directly contains `manifest.json`.
5. Pin Dubira and open its popup.
6. Enter the Gemini API key for the current browser session.
7. Open the video tab and click **Start dubbing this tab**.

The extension needs no Windows app, localhost, Python, FFmpeg, or virtual cable. `chrome.tabCapture` suppresses direct tab playback; Dubira recreates only the selected original/dub mix. This prevents the two voices from overlapping in **Dubbed audio only** mode.
...
2. Under **Dubira.exe**, leave **Output device** on `Default` or a physical headset.
3. Under **Dubira.exe**, set **Input device** to `Microphone (AMM Virtual Audio Device)`.
4. In the Dubira dashboard, select the matching AMM loopback as **Application audio input**.
5. Set Original to 0% and Dubbed to 100%.

![Exact Windows Volume Mixer routing](images/audio-routing-guide.png)

Do not route Dubira output to AMM Virtual. That feeds dubbed speech back into capture and creates echo/feedback.
...
- **Media Studio says FFmpeg is required:** install it with the WinGet command above and restart Dubira.
...
- **Close Dubira completely:** use **Quit app** in the dashboard top bar. Closing only the tab leaves the local process running.
...
- **Echo:** Dubira output and capture must not use the same virtual endpoint.
