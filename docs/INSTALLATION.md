...
The extension's faster synchronized engine uses Gemini 3.5 Live Translate directly. Its precise engine uses Groq Whisper for complete-utterance timestamps, the free Gemini text pool for translation, and Gemini 3.1 Flash Live for the selected voice. Both paths preserve natural generated PCM and align translated captions with audible dubbed speech. Chrome asks for the optional `api.groq.com` permission and keeps the Groq key only for the browser session.
