...
> Historical design, superseded by the Groq Whisper → Gemini text → Gemini Live file pipeline in Lingora 0.6.0. See `ARCHITECTURE.md` for the current implementation.
...
Each imported video gets a job directory under the existing Lingora data directory:
...
when other applications use the same project. Lingora therefore guarantees a lower
...
- Lingora input: `Microphone (AMM Virtual Audio Device)`
- Lingora output: `Default` or physical headphones/speakers
- Never route Lingora output back to the AMM virtual device.
