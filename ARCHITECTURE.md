...
- **Output boundaries:** on-page playback and synchronized playback/export own separate normalized output mixes. The synchronized mix defaults to dubbed audio only and never reads or mutates the on-page mix. Final export decodes selected source audio locally, combines it with timeline-aligned dub PCM through a paced WebCodecs audio track, and falls back to a standard Web Audio graph when track generation is unavailable. Both paths apply volume/ducking on one clock and record a new WebM; enabled subtitle channels are downloaded as separate SRT files.
...
- **Precise synchronized engine:** overlapping 12-second PCM windows go to Groq Whisper. `PreciseDubbingPipeline` holds incomplete utterances across windows, translates complete timestamped sentences through the strongest-first free Gemini/Gemma pool, and renders serialized turns through one persistent Gemini 3.1 Flash Live session. A processing frontier prevents the consumer from outrunning unfinished AI work while capture progress remains visible independently.
