...
- **Extension Gemini API key:** supplied by the user, stored only in Chrome session storage, not synced, and cleared when the browser fully exits. It is sent only to Google's official Gemini WebSocket/REST API after the user starts translation. Websites and the Avorythm project cannot read it.
- **Optional extension Groq API key:** requested only when the user chooses the precise synchronized engine. Chrome asks for the optional `api.groq.com` host permission, the key stays only in session storage, and short audio windows are sent directly to Groq for utterance text and timestamps. That text is sent directly to the Google Gemini text-model pool for translation, and the translation is sent to Gemini 3.1 Flash Live for speech rendering.
- **Preferences:** language, file/precise-mode voice, audio mix, floating-caption size/position/opacity, locale, recording choice, devices, and proxy settings are stored locally. The direct Gemini 3.5 Live Translate voice remains automatic.
...
- **Synchronized extension player:** after the user chooses this mode and presses Start, captured audio/video, the generated dub, and both subtitle timelines are written incrementally into Chrome's Origin Private File System so the independent player can pause, seek, fullscreen, recover after refresh, export the captured WebM, and retain SRT-ready caption timelines. The recording is not uploaded to Avorythm infrastructure. It remains until it is replaced by the next synchronized session or the user clears extension data/uninstalls; choosing Download creates a copy in Downloads. The selected engine sends audio/text directly to Google and, for precise mode, Groq solely for the requested translation. Separately enabling **Save four outputs** creates the two WAV and two SRT downloads and removes their temporary PCM files after finalization.
...
- **Gemini API Key اکستنشن:** توسط کاربر وارد می‌شود، فقط در Session Storage کروم می‌ماند، Sync نمی‌شود و با بسته‌شدن کامل مرورگر پاک می‌شود. کلید فقط بعد از شروع ترجمه به API رسمی WebSocket/REST جمنای فرستاده می‌شود؛ وب‌سایت‌ها و پروژهٔ Avorythm به آن دسترسی ندارند.
- **کلید اختیاری Groq در اکستنشن:** فقط وقتی کاربر موتور دقیق پلیر هماهنگ را انتخاب کند لازم است. Chrome همان موقع اجازهٔ اختیاری اتصال به `api.groq.com` را می‌خواهد؛ کلید فقط در Session Storage می‌ماند و پنجره‌های کوتاه صوتی برای متن جمله و timestamp مستقیم به Groq می‌روند. متن برای ترجمه مستقیم به مخزن مدل‌های متنی Gemini و ترجمه برای ساخت صدا به Gemini 3.1 Flash Live فرستاده می‌شود.
- **تنظیمات:** زبان، گویندهٔ فایل/موتور دقیق، میکس صدا، زبان رابط، ضبط، دستگاه‌ها و پروکسی فقط محلی هستند. صدای حالت مستقیم Gemini 3.5 Live Translate خودکار است.
...
- **پلیر هماهنگ اکستنشن:** وقتی کاربر این حالت را انتخاب و Start را می‌زند، صوت‌وتصویر تب، صدای دوبله و خط زمانی هر دو زیرنویس به‌تدریج در Origin Private File System کروم نوشته می‌شوند تا پلیر مستقل بتواند Pause، Seek، Fullscreen و بازیابی پس از Refresh داشته باشد، WebM ضبط‌شده را خروجی بدهد و خط‌های زمانی آمادهٔ SRT را نگه دارد. این ضبط به زیرساخت Avorythm آپلود نمی‌شود و تا شروع نشست هماهنگ بعدی، پاک‌کردن داده‌های اکستنشن یا Uninstall باقی می‌ماند؛ Download یک کپی در Downloads می‌سازد. موتور انتخابی داده را فقط برای ترجمهٔ درخواستی، مستقیم به Google و در حالت دقیق به Groq می‌فرستد. گزینهٔ جداگانهٔ **ذخیرهٔ چهار خروجی** دو WAV و دو SRT می‌سازد و PCM موقت آن‌ها پس از نهایی‌شدن حذف می‌شود.
