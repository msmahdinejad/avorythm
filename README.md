...
The synchronized player offers a fast Gemini 3.5 Live path and a precise Whisper → Gemini text pool → Gemini 3.1 Flash Live path. The precise path groups complete utterances, reuses one serialized Live voice session, preserves Gemini's natural PCM without pitch-changing resampling, and schedules translated captions behind the audible dub clock by a calibrated 2.5 seconds. Capture progress is independent from AI progress, recording can be stopped manually, media transitions such as YouTube autoplay are finalized automatically, and an active/finalized session survives a player-page refresh. On restricted networks, Chrome itself must route `api.groq.com` through the browser or system proxy; the extension checks this before recording starts.
