...
The synchronized player offers a fast Gemini 3.5 Live path and a precise Whisper → Gemini text pool → Gemini 3.1 Flash Live path. The precise path groups complete utterances, reuses one serialized Live voice session, preserves Gemini's natural PCM without pitch-changing resampling, and schedules translated captions on the audible dub clock. Capture progress is independent from AI progress, recording can be stopped manually, media transitions such as YouTube autoplay are finalized automatically, and an active/finalized session survives a player-page refresh.
